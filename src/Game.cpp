...
void GameMessage::to_bin()
{
	...
	else if (type == MessageType::CHECKEDTILE)
	{
		...
		alloc_data(sizeof(type) + sizeof(char) * NICK_SIZE + sizeof(int16_t) * 3 + sizeof(bool));
		char *tmp = _data;
		memcpy(tmp, &type, sizeof(type));
		tmp += sizeof(type);
		memcpy(tmp, nick.c_str(), sizeof(char) * NICK_SIZE);
...
void GameClient::setup()
{
	...
	turnIndicator->setDimensions(1920 * scale, 1080 * scale);
